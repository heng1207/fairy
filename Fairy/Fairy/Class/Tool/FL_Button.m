...
-(void)NewimageLeftAndTitleRight{
    
    //    获得按钮的图片的frame
    CGRect imageFrame = self.imageView.frame;
    //    设置按钮的图片的x坐标紧跟文本的后面
    imageFrame.origin.x = kScreenValue(5);
    //    获得按钮的文本的frame
    CGRect titleFrame = self.titleLabel.frame;
    //    设置按钮的文本的x坐标为0-－－左对齐
    titleFrame.origin.x = CGRectGetWidth(imageFrame) + kScreenValue(10);
    //    重写赋值frame
    self.titleLabel.frame = titleFrame;
    self.imageView.frame = imageFrame;
...
- (void)layoutSubviews{
    ...
    else if (_status == FLAimageLeftAndTitleRight){
        [self imageLeftAndTitleRight];
    }
    else if (_status == NewimageLeftAndTitleRight){
        [self NewimageLeftAndTitleRight];
    }
}
