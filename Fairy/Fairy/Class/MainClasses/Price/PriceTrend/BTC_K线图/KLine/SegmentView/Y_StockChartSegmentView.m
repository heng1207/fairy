...
- (void)setItems:(NSArray *)items
{
    ...
    for (NSString *title in items)
    {
        ...
        [btn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            if (self.isFullScreen) {
                    ...
                    make.height.equalTo(self).multipliedBy(1.0f/(count+1) );
                    make.width.equalTo(self);
                    if(preBtn)
                       {
    ...
    if (self.isFullScreen) {
        [fullScreenBtn setTitle:@"半屏" forState:UIControlStateNormal];
        [fullScreenBtn setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
    }else{
...
#pragma mark 底部按钮点击事件   MACD.KDJ选择按钮
