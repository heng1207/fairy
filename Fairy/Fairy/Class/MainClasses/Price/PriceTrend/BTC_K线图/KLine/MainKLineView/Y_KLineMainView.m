 ...
 *  MA15位置数组
 */
@property (nonatomic, strong) NSMutableArray *MA15Positions;
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.MA15Positions = @[].mutableCopy;
        ...
        self.maxPriceLab = maxPriceLab;
        [self addSubview:maxPriceLab];
        
        UILabel *minPriceLab = [[UILabel alloc]init];
...
//                lastDrawDatePoint = drawDatePoint;
    ...
    if (self.targetLineStatus == Y_StockChartTargetLineStatusBOLL) {
        ...
        //画MA15线
        MALine.MAType = Y_MA15Type;
        MALine.MAPositions = self.MA15Positions;
        [MALine draw];
...
- (NSArray *)private_convertToKLinePositionModelWithKLineModels {
    ...
    [kLineModels enumerateObjectsUsingBlock:^(Y_KLineModel * _Nonnull kLineModel, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (_targetLineStatus == Y_StockChartTargetLineStatusBOLL) {
            ...
            if(kLineModel.MA15)
            {
                if (minAssert > kLineModel.MA15.floatValue) {
                    minAssert = kLineModel.MA15.floatValue;
                }
                if (maxAssert < kLineModel.MA15.floatValue) {
                    maxAssert = kLineModel.MA15.floatValue;
                }
            }
            if(kLineModel.MA30)
            {
    ...
    [self.MA15Positions removeAllObjects];
    ...
    for (NSInteger idx = 0 ; idx < kLineModelsCount; ++idx)
    {
        ...
        if(unitValue > 0.0000001)
        {
            if(kLineModel.MA15)
            {
                ma15Y = maxY - (kLineModel.MA15.floatValue - minAssert)/unitValue;
            }
        ...
        NSAssert(!isnan(ma7Y) && !isnan(ma15Y)  && !isnan(ma30Y), @"出现NAN值");
        
        CGPoint ma7Point = CGPointMake(xPosition, ma7Y);
        CGPoint ma15Point = CGPointMake(xPosition, ma15Y);
        ...
        if(kLineModel.MA15)
        {
            [self.MA15Positions addObject: [NSValue valueWithCGPoint: ma15Point]];
        }
        if(kLineModel.MA30)
        {
