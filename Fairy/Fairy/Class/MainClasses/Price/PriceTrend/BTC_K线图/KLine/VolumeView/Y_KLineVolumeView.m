 ...
 *  Volume_MA15位置数组
 */
@property (nonatomic, strong) NSMutableArray *Volume_MA15Positions;
...
- (void)drawRect:(CGRect)rect
{
   ...
   if(self.targetLineStatus != Y_StockChartTargetLineStatusCloseMA){
        ...
        MALine.MAPositions = self.Volume_MA15Positions;
        [MALine draw];
...
- (NSArray *)private_convertToKLinePositionModelWithKLineModels:(NSArray *)kLineModels
{
    ...
    [kLineModels enumerateObjectsUsingBlock:^(Y_KLineModel *  _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if(model.Volume_MA15)
        {
            if (minVolume > model.Volume_MA15.floatValue) {
                minVolume = model.Volume_MA15.floatValue;
            }
            if (maxVolume < model.Volume_MA15.floatValue) {
                maxVolume = model.Volume_MA15.floatValue;
            }
        }
    ...
    [self.Volume_MA15Positions removeAllObjects];
    ...
    [kLineModels enumerateObjectsUsingBlock:^(Y_KLineModel *  _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if(unitValue > 0.0000001)
        {
            if(model.Volume_MA15)
            {
                ma15Y = maxY - (model.Volume_MA15.floatValue - minVolume)/unitValue;
            }
        }
        if(unitValue > 0.0000001)
        {
        ...
        if(model.Volume_MA15)
        {
            [self.Volume_MA15Positions addObject: [NSValue valueWithCGPoint: ma15Point]];
        }
        if(model.Volume_MA30)
        {
