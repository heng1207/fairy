...
- (NSNumber *)Volume_EMA15
{
    if(!_Volume_EMA15) {
        _Volume_EMA15 = @((2 * self.Volume + 14 * self.PreviousKlineModel.Volume_EMA15.floatValue)/16);
    }
    return _Volume_EMA15;
}
...
- (NSNumber *)MA15
{
    if (!_MA15) {
        ...
        if (index >= 14) {
            if (index > 14) {
                _MA15 = @((self.SumOfLastClose.floatValue - self.ParentGroupModel.models[index - 15].SumOfLastClose.floatValue) / 15);
            } else {
                _MA15 = @(self.SumOfLastClose.floatValue / 15);
            }
        }
    }
    return _MA15;
...
- (NSNumber *)Volume_MA15
{
    if([Y_StockChartGlobalVariable isEMALine] == Y_StockChartTargetLineStatusMA)
    {
        if (!_Volume_MA15) {
            ...
            if (index >= 14) {
                if (index > 14) {
                    _Volume_MA15 = @((self.SumOfLastVolume.floatValue - self.ParentGroupModel.models[index - 15].SumOfLastVolume.floatValue) / 15);
                } else {
                    _Volume_MA15 = @(self.SumOfLastVolume.floatValue / 15);
                }
            }
        }
    } else {
        return self.Volume_EMA15;
    }
    return _Volume_MA15;
}
...
- (Y_KLineModel *)PreviousKlineModel
{
    if (!_PreviousKlineModel) {
        ...
        _PreviousKlineModel.Volume_MA15 = @(0);
...
- (void) initWithDict:(NSDictionary *)dict
{
    ...
    {
        _Date = dict[@"time"];
        _Open = @([dict[@"price_o"] floatValue]);
        _High = @([dict[@"price_h"] floatValue]);
        _Low = @([dict[@"price_l"] floatValue]);
        _Close = @([dict[@"price_c"] floatValue]);
