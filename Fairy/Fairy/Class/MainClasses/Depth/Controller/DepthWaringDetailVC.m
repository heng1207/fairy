...
#import "XuXianView.h"
@interface DepthWaringDetailVC ()<UITableViewDataSource,UITableViewDelegate>
@property (nonatomic,strong)UITableView *tableView;
@property (nonatomic,strong)NSMutableArray *dataSoucre;
@end

@implementation DepthWaringDetailVC
...
-(void)GetDataView{
    ...
    NSString *str =  [NSString stringWithFormat:@"%@?token=%@",GetWarning,dict[@"token"]];
    ...
    [NetworkManage Get:str andParams:nil success:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue] == 200) {
            for (NSDictionary *dic in responseObject[@"data"]) {
                if ([dic[@"exceed"] isEqualToString:@"Y"]) {
