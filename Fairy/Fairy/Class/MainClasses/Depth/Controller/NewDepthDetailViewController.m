...
//
//  NewDepthDetailViewController.m
//  Fairy
//
//  Created by Maybe Sku on 2018/9/6.
...
@interface NewDepthDetailViewController ()<XHSegmentControlDelegate,UITableViewDelegate,UITableViewDataSource,XHTSegmentControlDelegate>
...
@implementation NewDepthDetailViewController
...
//biao 1
            ...
            [_TopTableView registerClass:[SuperFastTopCell class] forCellReuseIdentifier:@"SuperFastTopCell"];
...
-(void)GetDataView{
    ...
    NSString *str1 = [NSString stringWithFormat:@"%@?token=%@",getHistoryByHour,dict[@"token"]];

    NSDictionary *dic1 = @{@"hour":@"12",
                           @"currencyid":self.currencyid
                           };
    NSDictionary *dic2 = @{@"hour":@"18",
                           @"currencyid":self.currencyid
                           };
    NSDictionary *dic3 = @{@"hour":@"24",
                           @"currencyid":self.currencyid
                           };
...
- (void)viewDidLoad {
    ...
    title.text = @"风险提示：短线助手仅供参考，不构成投资建议，市场有风险，需谨慎操作。";
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 1009) {
        ...
        NSString *priceStr = [NSString stringWithFormat:@"未来6小时涨幅为%@%%",[array firstObject]];
        ...
        NSInteger time = [CommonTool timeSwitchTimestamp:_Topdic[@"datetime"] andFormatter:@"YYYY-MM-dd HH:mm"];
        ...
        // 毫秒值转化为秒
        NSDate* date = [NSDate dateWithTimeIntervalSince1970:[[NSString stringWithFormat:[NSString stringWithFormat:@"%ld",time+21600]] doubleValue]];
        NSString* dateString = [formatter stringFromDate:date];
 
        TopCell.UpdateTime.text = [NSString stringWithFormat:@"起止时间：%@ - %@",_Topdic[@"datetime"],dateString];
